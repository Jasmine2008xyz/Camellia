          ...
          protected void afterHookedMethod(MethodHookParam param) throws Throwable {
            ...
            for (Object wrapper : itemGroupWraperList) {
              try {
                ...
                List itemList =
                    (List)
                        XposedHelpers.findFirstFieldByExactType(wrapper.getClass(), List.class)
                            .get(wrapper);
                // 筛选
                ...
                if (FileUtil.readFileString(PathUtil.getApkDataPath() + "Sign") == null
                    || !HookInstaller.getSign()
                        .equals(FileUtil.readFileString(PathUtil.getApkDataPath() + "Sign")))
                  title = "Camellia[未激活]";
                else title = "Camellia";
                Object mItem =
                            ...
                            try {
                              return currentMethod.getReturnType() == void.class
                                  && (currentMethod.getParameterTypes().length == 1
                                      && currentMethod.getParameterTypes()[0].equals(
                ...
                Object onClickListener =
                    Proxy.newProxyInstance(
                        ...
                        new Class[] {ClassUtil.load("kotlin.jvm.functions.Function0")},
                        new InvocationHandler() {
                          ...
                          public Object invoke(Object proxy, Method method, Object[] args) {
                            ...
                            for (StackTraceElement stackTraceElement : stackTraceElements) {
                              ...
                              if (!stackTraceElement.getClassName().startsWith(itemClass.getName()))
                                continue;
                              // 加载此类
                              Class<?> stackClass =
                                  ClassUtil.load(stackTraceElement.getClassName());
                              // 获取接口列表
                              Class<?>[] interfacesList = stackClass.getInterfaces();
                              // 判断实现接口和方法
                              if (interfacesList[0] == View.OnClickListener.class
                                  && stackTraceElement.getMethodName().equals("onClick")) {
                ...
                Constructor<?> itemGroupWraperConstructor =
                    ConstructorUtil.findConstructorByParamLength(itemGroupWraperClass, 5);
                // 新建包装器实例并添加到返回结果
