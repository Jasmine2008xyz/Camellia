...
public class CatchInstance {
    public static void StartCatch() {
        ModuleController.mTasker.start();
    ...
    private static void FixUEH() {
        XposedHelpers.findAndHookMethod("java.lang.Thread", HookEnv.getHostClassLoader(), "setDefaultUncaughtExceptionHandler", ClassUtil.get("java.lang.Thread$UncaughtExceptionHandler"), new XC_MethodHook() {
            @Override
    ...
    public static void ThreadToast(String vText) {
        new Handler(ModuleController.mTasker.getLooper()).post(() -> {
            if (HookEnv.getContext() == null) return;
            Toast.makeText(HookEnv.getContext(), vText, Toast.LENGTH_LONG).show();
        });
    }
    ...
    static String InitPackVersion() {
        try {
            //获取普通apk版本信息
            PackageManager pm = HookEnv.getContext().getPackageManager();
    ...
    public static String IGetAvailableInfo(Thread thread) {
        ...
        thOut.append("ProcessName:").append((String)HookEnv.get("processName")).append("\n");
        ...
        thOut.append("AppType:").append("QQ").append("\n");
        thOut.append("ModuleVer:").append(BuildConfig.VERSION_NAME).append("\n");
        thOut.append("Time:").append(LogCat.GetNowTime()).append("\n");
    ...
    public static void ICatchEx(Thread thread, Throwable th) {
       ...
       // AutoReport.reportException("QQCrashReport", th, "");
        Toast.makeText(HookEnv.getContext(),
        "已阻止QQ闪退:\n" + th + "\n" +
                "日志已保存到路径:" + packer.getPath(),Toast.LENGTH_LONG).show();
