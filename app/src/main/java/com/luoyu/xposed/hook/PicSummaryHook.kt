...
class PicSummaryHook {
     ...
     //   if(!isEnabled) return
        ...
        XposedBridge.hookMethod(method, object : XC_MethodHook() {
            override fun beforeHookedMethod(param: MethodHookParam) {
                ...
                val isEnabled = ModuleController.Config.getBooleanData("图片外显/开关", false)
                if(!isEnabled) return
                ...
                // 使用反射设置字段值
                    ...
                    val setValue = ModuleController.Config.getStringData("图片外显/内容","No_Content")
                    if(ModuleController.Config.getBooleanData("图片外显/随机一言",false)) {
    ...
    fun onLongClick(){
        ...
        edit_view.setText(ModuleController.Config.getStringData("图片外显/内容",""))
        edit_view.setGravity(Gravity.CENTER)
        
        val btn = MaterialButton(context)
        btn.setOnClickListener {
            ModuleController.Config.putData("图片外显/内容",edit_view.getText().toString())
        }
        btn.setText("保存")
        ...
        switch.setChecked(ModuleController.Config.getBooleanData("图片外显/随机一言",false))
        switch.setOnCheckedChangeListener { _ , isChecked ->
            ModuleController.Config.putData("图片外显/随机一言",isChecked)
        }
